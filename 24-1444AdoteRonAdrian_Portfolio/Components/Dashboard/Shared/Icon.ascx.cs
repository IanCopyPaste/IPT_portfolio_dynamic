...
namespace _24_1444AdoteRonAdrian_Portfolio.Components.Dashboard.Shared
{
    // An inline SVG icon by name: <dash:Icon runat="server" Name="users" />. Inline rather than an
    // icon font or image, so it takes the surrounding text colour through currentColor.
    public partial class Icon : System.Web.UI.UserControl
    {
        // 24x24 paths from Material Icons (Apache 2.0).
        private static readonly Dictionary<string, string> Paths = new Dictionary<string, string>
        {
            ...
            { "collapse", "M17.59 18 19 16.59 14.42 12 19 7.41 17.59 6l-6 6 6 6Zm-6 0L13 16.59 8.42 12 13 7.41 11.59 6l-6 6 6 6Z" },
            ...
            { "chevron-down", "M16.59 8.59 12 13.17 7.41 8.59 6 10l6 6 6-6-1.41-1.41Z" },
        ...
        // Also used by components that build an icon from a property of their own.
        public static string Markup(string name, int size = 20)
        {
            string path;

            // Thrown rather than rendering nothing, so a misspelt name fails on the first page view.
            if (name == null || !Paths.TryGetValue(name, out path))
            {
                throw new ArgumentException("Unknown icon: " + name);
            }

            return "<svg class=\"adm-icon\" viewBox=\"0 0 24 24\" width=\"" + size + "\" height=\"" + size +
